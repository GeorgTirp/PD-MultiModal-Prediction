...
def main(folder_path, data_path, target, identifier, folds=10):
    ...
    Feature_Selection['features'] = [col for col in data_df.columns if col != Feature_Selection['target']]
    safe_path = os.path.join(folder_path, "results/level1/XGBoost")
    if not os.path.exists(safe_path):
        os.makedirs(safe_path)
    # Random Forest Model
     #XGBoost hyperparameters grid    
    param_grid_xgb = {
        'n_estimators': [25, 50, 100, 150],               # Keep trees small due to sample size
        'max_depth': [4, 5, 6],                     # Shallow trees to prevent overfitting
    ...
    model = XGBoostRegressionModel(
        ...
        -1,
        param_grid_xgb)
    metrics = model.evaluate(folds=folds, tune=True, nested=True, tune_folds=5, get_shap=True)
...
if __name__ == "__main__":
    ...
    main(folder_path, "data/BDI/level1/bdi_df.csv", "diff", "BDI", -1)
    main(folder_path, "data/MoCA/level1/moca_df.csv", "diff", "MoCA", -1)
    main(folder_path, "data/BDI/level1/bdi_df.csv", "ratio", "BDI", -1)
    main(folder_path, "data/MoCA/level1/moca_df.csv", "ratio", "MoCA", -1)
