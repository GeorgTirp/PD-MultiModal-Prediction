...
def main(
    ...
    # Always keep contextual covariates if present downstream
    context_cols = ["TimeSinceSurgery", "AGE_AT_OP", "TEST_DATUM"]
    required_cols.extend(context_cols)
    ...
    # Ensure every required column exists in the base dataframe (fill with NaN when absent)
    missing_base_cols = [col for col in ordered_required_cols if col not in data_df.columns]
    if missing_base_cols:
        if logging:
            logging.warning(f"Columns missing from base dataset – adding as NaN: {missing_base_cols}")
        for col in missing_base_cols:
            data_df[col] = np.nan

    base_cols_available = ordered_required_cols.copy()
    ...
    # Ensure a clean integer row index and add Pat_ID as a simple row-index identifier (1-based)
    data_df = data_df.reset_index(drop=True)
    data_df['Pat_ID'] = data_df.index + 1
    if logging:
        logging.info(f"Added Pat_ID column with {len(data_df)} entries (1-based index)")
    # Save the concatenated dataframe for inspection / reuse
    data_dir = os.path.dirname(data_path)
    out_joined_path = os.path.join(data_dir, "moca_updrs_joined.csv")
    os.makedirs(os.path.dirname(out_joined_path), exist_ok=True)
    data_df.to_csv(out_joined_path, index=False)
    if logging:
        logging.info(f"Saved joined DataFrame to: {out_joined_path}")
...
#        safe_best_hparams=True,
    ...
    inference_save_dir = os.path.join(safe_path, "inference_ppmi_ledd_new")
    model.inference(
