...
def main(
    ...
    sns.heatmap(data_df.select_dtypes(include='number').corr(method='spearman'), 
    ...
    plt.savefig(os.path.join(out,"all_features_corr.svg"), dpi=300, bbox_inches='tight')
    plt.close('all')
...
if __name__ == "__main__":
    ...
    for exp_info in exp_infos:
        main(
            ...
            data_path="data/BDI/level2/bdi_ledd.csv",
            feature_cols=exp_info['feature_cols'],
            target_col=exp_info['target_col'],
            outlier_cols=exp_info['outlier_cols'],
            out=f"results/{exp_info['exp_number']}_{exp_info['target_col']}_ledd/level2/NGBoost",
            folds=10,
            tune_folds=5,
            tune=False,
            members=2,
            uncertainty=False,
            filtered_data_path="filtered_BDI_ledd.csv",
        )
